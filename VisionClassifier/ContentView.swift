...
struct ContentView: View {
            ...
            ZStack {
                ...
                ForEach (vm.detections) {det in
                    ...
                    Text("\(det.label) \(Int(det.score * 100))%")
                        ...
                        .position(x: box.midX, y: box.midY - 10)
                }
    ...
    private func pixelRect(from norm: CGRect, in size: CGSize) -> CGRect {
        CGRect(x: norm.origin.x * size.width, y: norm.origin.y * size.height, width: norm.width * size.width, height: norm.height * size.height)
    }
}
